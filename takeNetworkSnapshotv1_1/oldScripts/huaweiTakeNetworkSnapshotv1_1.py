...
try:
    ...
    try:
        ...
        writeToLogfile(userArgs.pageName, f"HATA: Switch IP bilgileri alinamadi. Iptal ediliyor... {e}")
        exit(1)
    ...
    writeToLogfile(userArgs.pageName, f"HATA: On bilgiler alinamadi. Iptal ediliyor... Hata: {e}")
    exit(1)
...
for switchIP in switchList:
    try:
        ...
        if topologyRequested:
            ...
            with open("./lldpTemplate.textfsm") as templateFile:
                fsm = textfsm.TextFSM(templateFile)
                parsedOutput = fsm.ParseText(cdpOutput)
                headers = fsm.header

            cdpDict = getCdpItems(userArgs.pageName, [dict(zip(headers, row)) for row in parsedOutput])
...
if topologyRequested:
    try:
        ...
        networkGraph.show(f"./{userArgs.pageName.strip()}networkTopology.html", notebook=False)
        ...
        writeToLogfile(userArgs.pageName, f"HATA: Topoloji olusturulurken hata olustu. {traceback.extract_tb(e)}")
